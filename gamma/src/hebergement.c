...
GtkCellRenderer *renderer;
	...
	if(store == NULL)
	{
		...
		renderer = gtk_cell_renderer_text_new();
		column=gtk_tree_view_column_new_with_attributes(" Num_Capteur:",renderer,"text",NOMM,NULL);
		gtk_tree_view_append_column(GTK_TREE_VIEW(liste),column);
		...
		else
		{
			 ...
			 while ((fscanf(f," %s\t %s\t %s\t %s \n",jour,heure,nom,valeur)!=EOF)){
if(strcmp(valeur,"1")==0){
       gtk_list_store_append(store, &iter);
				gtk_list_store_set(store,&iter,JOUR,jour,HEURE,heure,NOMM,nom,VAL,valeur, -1);
}
			fclose(f);
			gtk_tree_view_set_model(GTK_TREE_VIEW(liste),GTK_TREE_MODEL(store));
			g_object_unref(store);
...
GtkCellRenderer *renderer;
	...
	if(store == NULL)
	{
		...
		renderer = gtk_cell_renderer_text_new();
		column=gtk_tree_view_column_new_with_attributes(" Num_Capteur:",renderer,"text",NOM1,NULL);
		gtk_tree_view_append_column(GTK_TREE_VIEW(liste),column);
		
		renderer = gtk_cell_renderer_text_new();
		column=gtk_tree_view_column_new_with_attributes(" Val:",renderer,"text",VAL1,NULL);
		gtk_tree_view_append_column(GTK_TREE_VIEW(liste),column);
...
if(strcmp(valeur,"1")==0){
       gtk_list_store_append(store, &iter);
				gtk_list_store_set(store,&iter,JOUR1,jour,HEURE1,heure,NOM1,nom,VAL1,valeur, -1);
...
/*
void affichage_alarm (GtkWidget *liste)
{
  	GtkCellRenderer *renderer;
	GtkTreeViewColumn *column;
	GtkTreeIter iter;
...
char val[20];	
  ...
  f = fopen("fumee.txt","r");
    ...
    if (f==NULL)
    {
      ...
      while ((fscanf(f," %s\t %s\t %s\t %s \n",&jour,&heure,&nomm,&val)!=EOF)){
if(strcmp(val,"1")==0){
       gtk_list_store_append(store, &iter);
				gtk_list_store_set(store,&iter,JOUR,jour,HEURE,heure,NOMM,nomm,VAL,val, -1);
}
